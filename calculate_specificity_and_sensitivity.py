...
def main():
    ...
    df_with_all_reference_spectra_and_merged_results_no_nan = \
        s_u_s.remove_all_decoy_and_nan_rows(df_with_all_reference_spectra_and_merged_results_in_fdr)
    ...
    print('Number of spectra identified in result but not in reference: ', len(set(df_only_in_result_identified_spectra.Title.tolist())))
    with open(str(path_to_result_file)+ '_sensitivity', 'w') as output:
        output.write('FDR:' + '\t' + str(options.fdr) + '\n')
